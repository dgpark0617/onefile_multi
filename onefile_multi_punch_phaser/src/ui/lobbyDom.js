...
function startNet(asHost) {
  if (MULTI_FILE_IOS_BLOCKED) {
    lobbyLog("아이폰 저장 파일에서는 멀티 불가. 웹사이트 /play/punch 사용");
    return;
  }
...
export function handleNetData(d) {
  ...
  if (d.type === "FRAME" && !sim.isHost) {
    ...
    sim.onRemoteInput(d.from, d.tick, d.input ?? d.turn);
...
function bindEdgeButton(el, edgeKey) {
  if (!el) return;
  el.addEventListener("pointerdown", (e) => {
    e.preventDefault();
    gameSession.edge[edgeKey] = true;
    el.classList.add("active");
...
export function initLobby(opts) {
  ...
  $("restartBtn")?.addEventListener("click", () => {
    $("overlay")?.classList.remove("show");
    if (gameSession.simulation?.solo) callbacks.onRestartSolo?.();
    else showLobby();
  });

  bindHoldButton($("btnLeft"), "left");
  bindHoldButton($("btnRight"), "right");
  bindEdgeButton($("btnForward"), "forward");
  bindEdgeButton($("btnPunch"), "punch");

  if (MULTI_FILE_IOS_BLOCKED) {
    $("iosFileWarn")?.classList.remove("hidden");
    $("btnHost") && ($("btnHost").disabled = true);
    $("btnJoin") && ($("btnJoin").disabled = true);
    lobbyLog("아이폰 + 저장 파일: 멀티 불가. /play/punch 사용");
  }
